...
window.setTimeout(()=>{
	...
	canvas = {"width":window.innerWidth,"height":window.innerHeight,"context":null}
	...
	points = [{"x":100,"y":100,"velX":0,"velY":0,"speed":1,"radius":4}];
	mousePos = {"x":canvas.width/2,"y":canvas.height/2};

	time = {"sinceLastStep":0,"lastStep":0,"startDate":Date.now()}
	stepTimer = 0;
...
function doPhysics(dt){
	...
	var now = dt;
	// var now = Date.now();
	time.sinceLastStep = Math.floor((now-time.lastStep));
	// console.log(time.sinceLastStep)
	// time.sinceLastStep = dt*1000;
...
function renderScene(){
	...
	for(var p = 0; p<points.length; p++){
		...
		if(point.radius < 1){
			...
			canvas.context.globalAlpha = 1/point.radius;
		}
		canvas.context.arc(point.x,point.y,point.radius+1,0,rad,false);
		canvas.context.fill();
